...
class TestAtenGtScalar(NKIPyTestBase):
    ...
    def test_gt_scalar_shapes_dtypes(self, shape, dtype, scalar):
        """Test aten.gt.Scalar with different shapes, dtypes, and scalar values."""

        def test_func(x):
            return torch.ops.aten.gt.Scalar(x, scalar)
    ...
    def test_gt_scalar_special_values(self):
        """Test gt.Scalar with special values."""

        def test_func(x):
            return torch.ops.aten.gt.Scalar(x, 0.0)
