...
class TestAtenMaxDim(NKIPyTestBase):
    ...
    def test_max_dim_basic(self, shape, dim, keepdim, dtype):
        """Test max.dim with different shapes, dimensions and dtypes."""

        def test_func(x):
            return torch.ops.aten.max.dim(x, dim, keepdim)

        # Create input tensor with varied values to ensure meaningful max operations
    ...
    def test_max_dim_tuple_return(self, shape, dim, dtype):
        """Test that max.dim returns a proper tuple of (values, indices)."""

        def test_func(x):
            values, indices = torch.ops.aten.max.dim(x, dim, False)
    ...
    def test_max_dim_identical_values(self, dtype):
        """Test max.dim behavior when all values along dimension are identical."""

        def test_func(x):
            return torch.ops.aten.max.dim(x, 1, False)
    ...
    def test_max_dim_extreme_values(self):
        """Test max.dim with extreme values (very large/small numbers)."""

        def test_func(x):
            return torch.ops.aten.max.dim(x, 0, True)
