...
class TestAtenLe(NKIPyTestBase):
    # Tests for torch.ops.aten.le.Tensor
    ...
    def test_le_tensor_basic(self, shape, dtype):
        """Test le.Tensor with different shapes and dtypes."""

        def test_func(x, y):
            return torch.ops.aten.le.Tensor(x, y)
    ...
    def test_le_tensor_broadcasting(self, shape_a, shape_b, dtype):
        """Test le.Tensor with different broadcasting scenarios."""
    ...
    def test_le_tensor_values(self):
        """Test le.Tensor with specific tensor values to verify correctness."""
    ...
    # Tests for torch.ops.aten.le.Scalar
    @pytest.mark.parametrize(
        "shape,scalar,dtype",
        [
    ...
    def test_le_scalar_basic(self, shape, scalar, dtype):
        """Test le.Scalar with different shapes, scalars and dtypes."""
    ...
    def test_le_scalar_values(self, scalar):
        """Test le.Scalar with various scalar values."""
        ...
        x = torch.tensor(
            [scalar - 1, scalar, scalar + 1, 0, -float("inf"), float("inf")],
            dtype=torch.float32,
        )
