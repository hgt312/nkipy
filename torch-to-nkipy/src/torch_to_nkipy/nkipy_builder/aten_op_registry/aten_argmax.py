...
@AtenOpRegistry.register("torch.ops.aten.argmax.default")
def argmax_default(node: fx.Node, computation_node: ComputationNode) -> None:
    """
    Handle argmax operation along a dimension.

    Maps PyTorch's torch.argmax(input, dim, keepdim) to tensor_apis.topk function
    with k=1.
    Returns the indices of maximum values along the specified dimension.

    PyTorch signature: torch.argmax(input, dim=None, keepdim=False) -> LongTensor

    Args:
        node: The FX node representing the argmax operation
    ...
    # Validate inputs
    if len(node.args) < 1:
        raise ValueError("argmax.default requires input tensor argument")
    ...
    # Handle case where dim is None (flatten and find global argmax)
    if dim is None:
        ...
        # Use topk on the reshaped tensor (dimension 0 after reshape to 1D)
        topk_result_var = temp_vars.next()

        topk_kwargs = {
            "k": "1",
            "axis": "0",  # Only dimension after reshape to 1D
            "is_ascend": "False",  # We want the largest value (max)
        }
        ...
        # Extract indices from topk result (we don't need values for argmax)
        ...
        # For global argmax, reshape to scalar (remove the k and axis dimensions)
        ...
        # Handle case where dim is specified - similar to max.dim logic
        ...
        if need_transpose:
            ...
            topk_input = transposed_input_var
            topk_axis = last_dim  # Now we always use the last dimension
        else:
            topk_input = input_tensor
            topk_axis = actual_dim

        # Step 2: Use topk with k=1 to get argmax indices
        topk_result_var = temp_vars.next()
        ...
        # Step 3: Extract indices from topk result (we only need indices for argmax)
        argmax_indices_var = temp_vars.next()

        # topk returns (values, indices), so extract indices
        ast_block.add_assignment(
            CodeGenerator.name_store(argmax_indices_var),
            CodeGenerator.name_load(f"{topk_result_var}[1]"),
        )
        ...
        if need_transpose:
            ...
            ast_block.add_numpy_call_assignment(
                ...
                func_name="transpose",
                args=[argmax_indices_var, inverse_perm_var],
            )

            argmax_indices_var = detransposed_indices_var
        ...
        if not keepdim:
            ...
            ast_block.add_numpy_call_assignment(
                ...
                func_name="reshape",
                args=[argmax_indices_var, new_shape_var],
            )

            final_indices_var = reshaped_indices_var
        else:
            final_indices_var = argmax_indices_var

    # Final step: Assign result (only indices, unlike max.dim which returns tuple)
    ast_block.add_assignment(
