...
@AtenOpRegistry.register("torch.ops.prims.convert_element_type.default")
def _convert_element_type(node: fx.Node, computation_node: ComputationNode) -> None:
    """
    Handle prims.convert_element_type (type cast).

    Signature: convert_element_type(input, dtype) — positional args, no kwargs.
    Appears in backward graphs generated by aot_autograd with dynamic shapes.
    """
    ast_block = computation_node.ast_code_block

    torch_dtype = node.args[1]
    numpy_dtype = torch_to_numpy_dtype_str(torch_dtype)
