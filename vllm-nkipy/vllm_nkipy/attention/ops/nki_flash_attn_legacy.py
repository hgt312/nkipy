...
def transform_block_tables_for_indirect_load(
    ...
    for partition_id in nl.affine_range(num_partitions):
        ...
        for i in nl.affine_range(num_loads):
            i_p = nl.arange(B_P_SIZE)[:, None]
            i_f = (
                partition_id * num_tiles_per_partition
...
def load_kv_tile_from_cache(
    ...
    for load_idx in nl.affine_range(num_loads):
        ...
        loaded = nl.load(
            kv_cache[0, block_tables[load_idx, i_p, large_k_tile_idx], i_f]
        )
    ...
    for load_idx in nl.affine_range(num_loads):
        loaded = nl.load(
            kv_cache[1, block_tables[load_idx, i_p, large_k_tile_idx], i_f]
        )
...
# @nki.jit
@NKIOpRegistry.register("mylib::neuron_paged_attn")
...
def reshape_and_cache_origin(
    ...
    head_indices = torch.arange(n_kv_head, dtype=torch.int32, device=key.device)
...
def reshape_and_cache(
    ...
    Returns:
        None: Updates the kv_cache tensor in-place
    """
    _, B, H, BS, D = kv_cache.shape

    # integer math for (block, offset)
    blk = torch.floor(torch.divide(slot_mapping, BS)).to(dtype=torch.int32)
    off = slot_mapping - BS * blk
    off = off.to(dtype=torch.int32)

    # linear position per (token, head): pos = blk*(H*BS) + head*BS + off
    base = blk * (H * BS) + off  # (T,)
    head_off = torch.arange(
        0, H * BS, BS, device=slot_mapping.device, dtype=torch.int32
    )  # (H,)
    flat_pos = base.repeat_interleave(H) + head_off.repeat(base.size(0))  # (T*H,)

    # flatten sources to match
    key_flat = key.reshape(-1, D)  # (T*H, D)
    value_flat = value.reshape(-1, D)

    # single-dim dynamic writes
    cache_flat = kv_cache.reshape(2, B * H * BS, D)  # view
    cache_flat[0].index_copy_(0, flat_pos, key_flat)
    cache_flat[1].index_copy_(0, flat_pos, value_flat)


def reshape_and_cache2(
    key: torch.Tensor,
    value: torch.Tensor,
    kv_cache: torch.Tensor,
    slot_mapping: torch.Tensor,
) -> None:
    tmp = kv_cache.permute(0, 1, 3, 2, 4).contiguous()
    ...
    # s = torch.stack([key_cache, value_cache])
    # tmp = s.reshape(tmp.shape)
    tmp = tmp.permute(0, 1, 3, 2, 4).contiguous()

    kv_cache.copy_(tmp)


def reshape_and_cache3(
    key: torch.Tensor,
    value: torch.Tensor,
    kv_cache: torch.Tensor,
    slot_mapping: torch.Tensor,
    ...
    # Calculate indices with explicit floor division
    block_indices = torch.floor(torch.divide(slot_mapping, block_size)).to(
        dtype=slot_mapping.dtype
    )
    block_offsets = slot_mapping - block_size * block_indices
