...
def check_input_shape(
    *,
    ...
    assert seqlen_q % PREFILL_INNER_Q_TILE_SIZE == 0, (
...
def merge_decode_buffer(
    ...
    for i in nl.affine_range(NUM_TILES):
        base_offset_iota = nisa.iota(
            nl.arange(TILE_SIZE)[None, :] + i * TILE_SIZE, dtype=nl.int32
        )
...
def flash_paged_attention_varlen(
    *,
    ...
    IO tensor layouts:
      ...
      - value_cache:
          (max_num_blocks, n_kv_heads, block_size, d)
      - prefill_tile_q_indices:
          (max_num_prefill_tiles, large_tile_size_q)
      - prefill_tile_block_tables:
          (max_num_prefill_tiles, num_block_per_large_tile)
      - prefill_tile_masks:
          (B_P_SIZE, max_num_prefill_tiles,
      ...
      - decode_tile_block_tables:
          (max_num_decode_tiles, num_block_per_large_tile)
      - decode_tile_masks:
          (B_P_SIZE, max_num_decode_tiles,
           large_tile_size_k // B_P_SIZE)
      - decode_num_dynamic_loop_steps: (1, 1)
      - active_mask: (seq_q, seq_q)
      - decode_last_tile_indices: (max_batch_size, 1)
      - decode_q_update_pred:
          None or (max_num_decode_tiles, 1)
    ...
    if decode_q_update_pred is None:
        ...
        decode_q_update_pred_hbm = decode_q_update_pred
    decode_q_update_pred = decode_q_update_pred_hbm.reshape(
        (DECODE_MAX_NUM_TILE // dynamic_loop_unroll_factor, dynamic_loop_unroll_factor)
    )
    ...
    if return_accum_buffers:
        pass
        o_buffer = nl.ndarray((b, seqlen_q, h, d), dtype=acc_type, buffer=nl.shared_hbm)
        l_buffer = nl.ndarray((b, seqlen_q, h, 1), dtype=acc_type, buffer=nl.shared_hbm)
        m_buffer = nl.ndarray((b, seqlen_q, h, 1), dtype=acc_type, buffer=nl.shared_hbm)
        write_back_accum_buffers(
            olm_buffer=olm_buffer,
            o_buffer=o_buffer,
            l_buffer=l_buffer,
            m_buffer=m_buffer,
    ...
    for i in nl.affine_range(num_active_tiles):
        nl.store(
            o_buffer[
                ...
                nl.ds(head_id * q_h_per_k_h, q_h_per_k_h),
                :,
            ],
        ...
        nl.store(
            ...
            olm_buffer_sbuf[:, i, :, nl.ds(B_D_SIZE, 1)],
        )
        nl.store(
            m_buffer[
                batch_id,
            ...
            olm_buffer_sbuf[:, i, :, nl.ds(B_D_SIZE + 1, 1)],
        )
