...
def load_indices(indices_hbm):
    ...
    To map num_tiles to SBUF partition dimension, this
    function automatically partitions num_tiles with
    partition_size set to min(num_tiles, B_P_SIZE=128)
...
def load_indices_for_loop_step(
    ...
    """
    Load a 2D indices array with dim 0 range
    [loop_index * step_size, (loop_index + 1) * size)
    from HBM with start offset to SBUF

    To map num_tiles to SBUF partition dimension, this
    function automatically partitions num_tiles with
    partition_size set to min(step_size, B_P_SIZE=128)

    Output SBUF tensor shape:
      [par_dim(partition_size),
       ceil_div(size, partition_size), num_indices]
    """
...
def transform_to_vector_dge_layout(
    ...
    """
    Transpose an tile of shape [tile_size, num_indices] so
    that num_indices is mapped to partition dimension and
    perform partition with
    partition_size=min(num_indices, B_P_SIZE=128)
