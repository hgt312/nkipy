...
if NUM_NEURON_CORES is None:
    ...
    product_name = result.stdout
    if product_name.startswith("trn1"):
        NUM_NEURON_CORES = 32
    elif product_name.startswith("trn2"):
        NUM_NEURON_CORES = 64
    else:
        assert 0, product_name
...
def assign_neuron_cores(worker_id, requested_num_cores=1):
    num_cores = NUM_NEURON_CORES
    if worker_id == "master":
        worker_id = 0
    else:
    ...
    assert num_cores_per_worker >= requested_num_cores
    core_start = worker_id * num_cores_per_worker
    if num_cores_per_worker > 1:
        core_end = core_start + requested_num_cores - 1
...
def sample_input_tensors(
    ...
    k_active = torch.zeros(sum(query_lens), num_kv_heads, head_size, dtype=dtype)
    v_active = torch.zeros(sum(query_lens), num_kv_heads, head_size, dtype=dtype)
    block_tables = torch.randperm(cache_size, dtype=torch.int32)
    ...
    for i in range(batch_size):
        for j in range(query_lens[i]):
            k_active[b_start_loc[i] + j].copy_(
                key[b_seq_start_loc[i] + b_ctx_len[i] + j]
            )
            v_active[b_start_loc[i] + j].copy_(
                value[b_seq_start_loc[i] + b_ctx_len[i] + j]
            )
        cur_ctx = 0
        block_id = 0
        while cur_ctx < seq_lens[i]:
            start_loc = b_seq_start_loc[i] + cur_ctx
            if cur_ctx + block_size > seq_lens[i]:
    ...
    return query, k_active, v_active, k_cache, v_cache, block_tables, key, value
...
def save_kernel_tensors_as_npy(
    artifact_dir,
    *,
    is_torch_tensor,
    golden=None,
    **kwargs,
    ...
    if golden is not None:
        assert "golden" not in kwargs, (
            "Name conflict! Already has a tensor called golden"
        )
        kwargs["golden"] = golden
    tensor_map = []
    numpy_tensor_dir = "kernel_tensors"
    os.makedirs(os.path.join(artifact_dir, numpy_tensor_dir), exist_ok=True)
    if is_torch_tensor:
        kwargs = convert_torch_tensor_to_numpy(kwargs, use_bf16=True)
    for arg_name in kwargs:
        arg = kwargs[arg_name]
        if isinstance(arg, np.ndarray):
            relative_path = os.path.join(numpy_tensor_dir, f"{arg_name}.npy")
            np.save(os.path.join(artifact_dir, relative_path), arg)
            tensor_map.append(f"{arg_name} {relative_path}")
    with open(f"{artifact_dir}/kernel_tensor_mapping.txt", "w") as f:
        f.write("\n".join(tensor_map))
        f.write("\n")
