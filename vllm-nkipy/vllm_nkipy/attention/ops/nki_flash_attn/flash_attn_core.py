...
def _flash_attention_core(
    ...
    m_previous = olm_buffer[:, B_D_SIZE + 1]  # nl.copy(m_buffer[:, 0])
...
def _flash_attention_core_kq_matmul(
    ...
    if large_tile_idx + 1 < NUM_LARGE_TILE:
        ...
        for load_idx in nl.affine_range(num_loads):
            ...
            loaded = nl.ndarray((B_P_SIZE, block_size * B_D_SIZE), dtype=kv_cache.dtype)
            loaded[i_p, i_f] = nl.load(
                kv_cache[0, block_tables_sbuf[i_p, load_idx, large_tile_idx + 1], i_f]
            )
        ...
        for load_idx in nl.affine_range(num_loads):
            ...
            if kernel_dtype == kv_cache.dtype:
                v_load_buffer[
                    ...
                    kv_cache[
                        1, block_tables_sbuf[i_p, load_idx, large_tile_idx + 1], i_f
                    ],
                )
            else:
                loaded = nl.ndarray(
                    (B_P_SIZE, block_size * B_D_SIZE), dtype=kv_cache.dtype
                )
                loaded[...] = nl.load(
                    kv_cache[
                        1, block_tables_sbuf[i_p, load_idx, large_tile_idx + 1], i_f
    ...
    # nl.device_print("flash_attn_core.py: q_update_pred_sbuf[:, large_tile_idx]=", q_update_pred_sbuf[:, large_tile_idx])
    # nl.device_print("flash_attn_core.py: q_update_pred_broadcast[:, large_tile_idx]=", q_update_pred_broadcast[:, large_tile_idx])
    # nl.device_print("flash_attn_core.py: o_buffer_sbuf[0, large_tile_idx, 0]=", o_buffer_sbuf[:8, large_tile_idx, 0])
    nisa.tensor_copy_predicated(
    ...
    # nl.device_print("flash_attn_core.py: o_buffer_sbuf[0, large_tile_idx+1, 0]=", o_buffer_sbuf[:8, large_tile_idx+1, 0])
