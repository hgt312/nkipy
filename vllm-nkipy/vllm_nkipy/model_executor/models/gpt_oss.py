...
# Use vllm-nkipy's modified FusedMoE
import vllm_nkipy
from vllm_nkipy import envs as nkipy_envs
from vllm_nkipy.compile import NKIPY_BACKEND, local_compile
from vllm_nkipy.model_executor.layers.fused_moe.layer import FusedMoE

RMSNorm = local_compile(
    backend=NKIPY_BACKEND, name="RMSNorm", fullgraph=True, dynamic=False
)(RMSNorm)
VocabParallelEmbedding = local_compile(
...
@local_compile(
    backend=NKIPY_BACKEND, name="OAIAttention", fullgraph=True, dynamic=False
)
class OAIAttention(nn.Module):
    def __init__(
        ...
        self.q_size = max(1, self.num_attention_heads // tp_size) * self.head_dim
        self.kv_size = max(1, self.num_key_value_heads // tp_size) * self.head_dim
        ...
        self.num_local_key_value_heads = max(1, config.num_key_value_heads // tp_size)
...
@local_compile(backend=NKIPY_BACKEND, name="MLPBlock", fullgraph=False, dynamic=False)
class MLPBlock(torch.nn.Module):
    def __init__(
        ...
        self.experts = vllm_nkipy.model_executor.layers.fused_moe.layer.FusedMoE(
        ...
        logger.info(f"[DEBUG] {FusedMoE=}")
        logger.info(f"[DEBUG] {self.experts=}")
        logger.info(
            "[DEBUG] Type: %s.%s",
            type(self.experts).__module__,
            type(self.experts).__qualname__,
        )
...
class GptOssForCausalLM(nn.Module):
    ...
    def _load_weights_other(
        ...
        if dp_size > 1 and not nkipy_envs.VLLM_NKIPY_MOE_2D:
            ...
            moe_per_rank_intermediate_size = cdiv(
                intermediate_size, moe_parallel_config.tp_size
            )
            moe_tp_rank_start = (
                moe_parallel_config.tp_rank * moe_per_rank_intermediate_size
            )
            moe_tp_rank_end = min(
                (moe_parallel_config.tp_rank + 1) * moe_per_rank_intermediate_size,
                intermediate_size,
            )
            moe_heads_per_rank = (
                self.model_config.num_attention_heads // moe_parallel_config.tp_size
            )
            moe_head_start = moe_parallel_config.tp_rank * moe_heads_per_rank
            logger.info(
                "%s, per_rank_int=%s, start=%s, end=%s, "
                "heads=%s, head_start=%s",
                moe_parallel_config,
                moe_per_rank_intermediate_size,
                moe_tp_rank_start,
                moe_tp_rank_end,
                moe_heads_per_rank,
                moe_head_start,
            ...
            moe_per_rank_intermediate_size = per_rank_intermediate_size
            moe_tp_rank_start = tp_rank_start
            moe_tp_rank_end = tp_rank_end
            moe_heads_per_rank = heads_per_rank
            moe_head_start = head_start

        for name, weight in weights:
            if ".experts.gate_up_proj" in name and "bias" not in name:
                # Handle MLP gate and up projection weights
                new_name = name.replace(".experts.gate_up_proj", ".experts.w13_weight")
                ...
                # Handle MLP down projection weights
                new_name = name.replace(".experts.down_proj", ".experts.w2_weight")

                if use_ep:
                    narrow_weight = weight[
                        ep_rank_start:ep_rank_end, moe_tp_rank_start:moe_tp_rank_end, :
                    ]
                else:
                    narrow_weight = weight[:, moe_tp_rank_start:moe_tp_rank_end, :]
                narrow_weight = narrow_weight.permute(0, 2, 1).contiguous()
                param = params_dict[new_name]
                ...
                shard_id = (
                    "q" if "q_proj" in name
                    else "k" if "k_proj" in name
                    else "v"
                )
                name = name.replace("self_attn", "attn")
                param_name = name.replace(
                    f"{shard_id}_proj", "qkv"
                )
                param = params_dict[param_name]
                weight_loader = param.weight_loader
                weight_loader(
                    param, weight, loaded_shard_id=shard_id
                )
                loaded_params.add(param_name)
            else:
                # Handle all other weights with renaming
